...
function integrateFloatingButtonsIntoWpContent($dd_floating_button_for_display,$content,$ddFloatDisplay){
	...
	if($floatButtonsContainer != DD_EMPTY_VALUE){
		...
		$dd_override_start_anchor_id = isset($dd_override_start_anchor_id) ? $dd_override_start_anchor_id : '';

		$floatingJSOptions .= 'var dd_override_start_anchor_id = "'. $dd_override_start_anchor_id . '";';

		$dd_override_top_offset = isset($dd_override_top_offset) ? $dd_override_top_offset : '';

        $floatingJSOptions .= 'var dd_override_top_offset = "'. $dd_override_top_offset . '";';
        $floatingJSOptions .= '</script>';
		$floatingJS = '<script type="text/javascript" src="https://www.dubicars.com/news/wp-content/plugins/digg-digg/js/diggdigg-floating-bar.js?ver=' . DD_VERSION . '"></script>';
