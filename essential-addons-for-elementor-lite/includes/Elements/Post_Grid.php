...
class Post_Grid extends Widget_Base
{
    ...
    protected function render()
    {
        ...
        $dubiNoPost = '<p class="no-posts-found">No posts found!</p>';
        if (ICL_LANGUAGE_CODE === 'ar') {
        $dubiNoPost = '<p class="no-posts-found">لم يتم العثور على أية منشورات!</p>';
        ...
        if(file_exists($template)){
            ...
            if ( $query->have_posts() ) {
                ...
                _e($dubiNoPost, 'essential-addons-for-elementor-lite');
            }
            wp_reset_postdata();
        } else {
