...
class ECStarsRating {
	...
	public function headScript() {
		wp_enqueue_script('fp_rating', get_template_directory_uri().'/js/fp-rating.js',array('jquery'));
		
		wp_localize_script( 'fp_rating', 'ec_ajax_data', array(
			...
			'messages' => array(
				'success' => __('You have voted successfully.', 'fairpixels'),
				'previously_voted' => __('You have already voted.', 'fairpixels'),
				'request_error' => __('There was some problem.', 'fairpixels'),
				'unknown' => __('An error occurred.', 'fairpixels')
			)
		));
	}
...
<div class="ec-stars-outer">
	...
	<div class="ec-stars-wrapper" data-post-id="<?php echo esc_attr( $post->ID ); ?>">
		<div class="ec-stars-overlay" style="width: <?php echo (100 - $result * 100 / 5) ?>%"></div>
		<a href="#" data-value="1" title="<?php _e('Rate it 1 star', 'fairpixels'); ?>"><i class="fa fa-star"></i></a>
		<a href="#" data-value="2" title="<?php _e('Rate it 2 stars', 'fairpixels'); ?>"><i class="fa fa-star"></i></a>
		<a href="#" data-value="3" title="<?php _e('Rate it 3 stars', 'fairpixels'); ?>"><i class="fa fa-star"></i></a>
		<a href="#" data-value="4" title="<?php _e('Rate it 4 stars', 'fairpixels'); ?>"><i class="fa fa-star"></i></a>
		<a href="#" data-value="5" title="<?php _e('Rate it 5 stars', 'fairpixels'); ?>"><i class="fa fa-star"></i></a>
	</div>
	
	<div class="ec-stars-value">
		<span class="ec-stars-rating-value"><?php
			echo is_int($result) ? $result : number_format($result, 1);
		?></span> (<span class="ec-stars-rating-count votes"><?php echo esc_html( $votes ); ?></span> <?php echo __('votes', 'fairpixels') ?>)
	</div>
		
	<div style="display:none" itemscope itemtype="http://schema.org/Product">
		<span itemprop="name"><?php the_title(); ?></span>
		<div itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
			<span itemprop="ratingValue"><?php echo esc_attr( $result ); ?></span>
			out of <span itemprop="bestRating">5</span>
			based on <span itemprop="ratingCount"><?php echo esc_attr( $votes ); ?></span> user ratings
		</div>     
    </div>	
...
function ec_stars_rating_single() {
	global $post;
	$rating = get_post_meta($post->ID, 'ec_stars_rating', true);
	$votes = get_post_meta($post->ID, 'ec_stars_rating_count', true);
	if( $rating === '' ) {
